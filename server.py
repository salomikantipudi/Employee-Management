...
database = os.path.join(os.path.dirname(__file__), "jaya.db")
...
def home():
     ...
     return render_template("home.html",theme=theme,username=username,fullname=fullname)
...
def employees():
    ...
    cursor = connection.cursor()
    cursor.execute("""
        SELECT employeename, email, phone, department, salary, address, joining
        FROM employees
...
##-------EDIT EMPLOYEE:GET------##
@app.route("/edit-employee/<path:email>")
